...
/*
 * Moves the given container in the given direction and creates a new split instead of swaping two containers (D_LEFT, D_RIGHT,
 * D_UP, D_DOWN).
 *
 */
        ...
        if (same_orientation == con->parent) {
            ...
            if (swap) {
                ...
                if (con->parent->layout == L_TABBED || con->parent->layout == L_STACKED) {
                    /* If the parent is tabbed or stacked just swap the containers */ 
                    DLOG("Swapping with sibling.\n");
                    if (direction == D_LEFT || direction == D_UP) {
                        TAILQ_SWAP(swap, con, &(swap->parent->nodes_head), nodes);
                    } else {
                    ...
                    /* If the parent is a horizontal or vertical split create an 
                     * opposing split inside and move both containers into it */
                    DLOG("create new container (HORIZ or VET) and move con and sibling to it");
                    if (direction == D_LEFT || direction == D_RIGHT) {
                        tree_split(swap,VERT);
                        tree_move(con,direction);
                        return;
                    } else {
                        tree_split(swap,HORIZ);
                        tree_move(con,direction);
                        return;
                    }
                }
